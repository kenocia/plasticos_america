...
class NetworkPrinter(models.Model):
    ...
    def send_tcp_raw_binary(self, data, port=None, timeout=30):
        """Binario tal cual por TCP (ZPL/Zebra, puerto 9100). También disponible sin kc_mrp_wizard."""
        self.ensure_one()
        ...
        try:
            ...
            sock.close()
            _logger.info(
                'KC Plasticasa: RAW binario enviado %s bytes a %s:%s',
            ...
            _logger.exception('send_tcp_raw_binary (kc_plasticasa)')
            return {
                'success': False,
